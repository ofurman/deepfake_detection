...
for i in tqdm(range(fake_audio.shape[0]), total=fake_audio.shape[0]):
    ...
    #     continue
    fake_audio_unit = fake_audio[i].astype(float) / (np.linalg.norm(fake_audio[i].astype(float), ord=2, axis=-1, keepdims=True))
    fake_vid_unit = fake_vid[i].astype(float) / (np.linalg.norm(fake_vid[i].astype(float), ord=2, axis=-1, keepdims=True))
    fake_audio_video.append(np.percentile((fake_audio_unit * fake_vid_unit).sum(axis=1), lambda_percentile))

print(fake_audio_video)
